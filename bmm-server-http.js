    ...
    if (name === 'get_broker_context') {
      ...
      const emailBacklog = await query(
        `SELECT COUNT(*) FILTER (WHERE processed = false AND is_outgoing = false) AS count,
                ...
                COUNT(*) FILTER (WHERE processed = false AND is_outgoing = true) AS out_count,
        ...
        content: [{
          ...
          text: JSON.stringify({
            ...
            session_summary: {
              ...
              unprocessed_outbound_emails: hasIngest ? parseInt(emailBacklog.rows[0].out_count) : null
            },
            ...
            standing_rules: {
              ...
              outbound_sweep: "Sent mail must not black-hole either (spec 2026-07-23). session_summary.unprocessed_outbound_emails counts YOUR outgoing rows still processed=false (email_archive WHERE processed=false AND is_outgoing=true AND deleted_at IS NULL AND account='" + profile.email + "'). The nightly outbound sweep auto-links sent mail to known contacts (interaction + processed) and auto-processes pure internal broker chatter, so what remains is mail YOU sent to someone with no CRM contact. If the count is nonzero, render it as a line in the brief and ASK: for each unknown recipient, propose either a new contact (a real prospect/referral/lender the broker emailed deserves a record - broker confirms before creation) or mark processed with a processing_note stating why no contact is warranted (vendor, personal, one-off). Never leave a counterparty the broker is actively emailing without a CRM record for more than a week." } : {})
            }
          }, null, 2)
        }]
      };
